...
- `hooks/lib/wake-count.jq`, shared by the three Stop guards: counts the background work and crons that will wake the session. `teammate`, `dream`, `auto-mode scan` and a websocket monitor never count. In `strict` mode, used by the run guards, only an allowlist counts: subagent, workflow, MCP task, cloud session, and a shell whose command is on a positive list of finite jobs (a test, build, analyze, lint or install run, `gh run watch`) and carries no `until` or `while` loop, no recognised log follow and no recognised long-lived process. An unknown task type and a recurring cron do not count, so an unrecognised command errs toward one extra block rather than a run left unguarded. In `loose` mode, used by the announce guard, a wait loop or log follow counts and only a recognised long-lived process with neither is left out. Known limits, stated in the file: the payload carries no `persistent` flag for a `Monitor`, and the binary clips `command` at 1000 characters.

### Changed

- `stop-guard.sh` and `stop-guard-auto.sh` let the turn end while counted work is in flight, without spending block budget; the guard re-arms once the work lands. Both block reasons say no worker is running, so a sleep or polling loop is the wrong move; `stop-guard.sh` adds that a finished worker's notification may still be queued behind the next tool call.
- `stop-guard-announce.sh` uses the same count, so a background dev server no longer switches it off for the session, and a `Monitor` tailing a log still counts. It drops balanced double-quoted and curly-quoted spans before matching, so a quoted trigger word is a mention rather than an offer.
- `ac:execute`, `ac:plan` and `ac:auto` list "a one-line wait status while background workers you spawned still run" as a legitimate turn ending in their standing rules, and say how to wait: end the turn, let each task-notification start the next one, never a `sleep` loop or repeated reads of evidence files.
- The global CLAUDE.md section counts four turn endings, the fourth a wait on background work that will notify you, and names holding the turn open with a polling loop as a stall.
- `/ac:init-project` follows the Claude Code 2.1.280 setup model. It shows everything it would write as one proposal and takes one approval before any file or settings write, the shape of the built-in `/init` under `CLAUDE_CODE_NEW_INIT=1`, in place of a question per surface. A new settings step writes `permissions.deny` rules for the secret-shaped files it found (team-shared, with gitignore negation for templates such as `.env.example`) and `permissions.allow` rules for the project's own non-destructive commands (local), and never `defaultMode`. The hook can be a formatter, a linter, or both in one script, since matching hooks run in parallel. One `ac:oracle` pass reviews the drafts against the project before they are written (`--no-oracle` skips it), told which model and prompt shape the readers run. Nothing, including the `CLAUDE.local.md` line in `.gitignore`, is written before the approval. A root `AGENTS.md` is imported with `@AGENTS.md` rather than restated, because Claude Code reads it on its own only when no `CLAUDE.md` exists and never in a session without feature-flag fetching; `.devin/rules/`, `.windsurf/rules/` and `.claude/skills/` join the discovery inventory; a multi-step procedure becomes a reported skill candidate instead of CLAUDE.md prose; and the closing step points at the `/doctor` CLAUDE.md trim check.
- `hook-wiring.md` covers the format-on-edit variant and the one-script rule for a formatter plus a linter.

### Fixed

- Stale line anchors into the Stop guards in `ac:auto`, `criteria-schema.md` and `plan-check.ts`, and into `ac:claude-md-rules-creator` in `/ac:init-project`.
- Path-scoped rules written by `ac:claude-md-rules-creator` and `/ac:init-project` loaded at every launch. The rule template put its attribution comment above the `---` frontmatter, and Claude Code parses frontmatter only at byte 0, so `paths:` was read as body text; verified on 2.1.280 with two otherwise identical rules. The template, the SKILL.md example and the pre-flight checklist now put the comment after the frontmatter, `anti-patterns.md` and the loader troubleshooting name the failure, and `/ac:init-project` flags an existing rule with content above its frontmatter and offers the one-line repair under every mode. A scan on 2026-09-23 found the shape in 27 of 123 rule files across six local projects.
- `ac:explore` could change the repository it was searching. In a live `/ac:init-project` run on 2026-09-23 a Haiku discovery agent ran `prettier --write src/api/users.js --check` and rewrote two tracked files, and two agents opened `.env` although the brief said "path only". `pretooluse-explore-budget.sh` now denies, inside `ac:explore` only, a `Read` or `Grep` of a secret-shaped file (templates such as `.env.example` stay readable) and any shell command that names one outside an existence check, plus the common write forms: a formatter or fixer in write mode, a package install, a destructive git command (including `git -C`), `rm`/`mv`/`cp`-style changes, `sed -i`, `tee` and redirects into files. Each check is anchored at the command word of each simple command, so a search that only mentions a tool is not denied, and a denied call does not count against the budget; the body and the init-project briefs say the same, with `--version` as the most a tool is run.
- `pretooluse-explore-budget.sh` kept its per-spawn call counters in the target project's `.ac/state/agent-budget/`, which left an untracked, unignored `.ac/` directory in every repository an `ac:explore` ran in, including ones that never used ac and a `/ac:init-project --dry-run` that writes nothing. The counters now live under `${TMPDIR:-/tmp}/ac-agent-budget/`, keyed by agent id.
- `/ac:init-project` no longer calls anthropics/claude-code#82482 open: it was closed as stale on 2026-09-22, not fixed, and path-scoped rules still load on the `Read` tool only, which the 2.1.280 binary confirms.

...
| Tier    | Model                          | SWE-bench Verified | SWE-bench Pro | Terminal-Bench | When to assign | Write style |
|---------|--------------------------------|--------------------|---------------|----------------|----------------|-------------|
| quick   | claude-haiku-4-5-20251001      | 73.3%              | not reported  | 41.75%         | 1 isolated file, mechanical: config edit, rename, scaffold, single-file fix | File path + 1-2 sentence outcome + optional pattern reference. No detail past "what to produce". |
| junior  | claude-sonnet-5                | see model-tiers.md | 63.2%        | 80.4%          | Standard implementation, business logic, pattern application, most multi-file pattern work. The default tier; Sonnet 5 is near-Opus, so do not jump to senior for file count alone. | Outcome + pattern reference (file:line) + Must NOT scope guardrails. Sonnet 5 reads broad context and avoids duplicating shared logic. |
| senior  | claude-opus-4-8                | 88.6%              | 69.2%         | 74.6%          | Genuinely cross-layer changes, architecture, migration, long-horizon or critical work, complex edges, self-verification | High-level intent + architectural constraint + cross-cutting concerns + acceptance criterion. Opus 4.8 designs the solution; do not prescribe low-level code. |
...
- senior — bad: `"First create A, then B imports A, then C imports B, then D imports C."` Opus 4.8 designs the order. Write `"Implement event-driven dispatch matching the pattern at dispatcher/core.ts:142; preserve the at-least-once delivery invariant."`.

---
description: Executor for plans approved by `/ac:plan`. Loops wave by wave on the main thread until the plan is complete, spawning tier-routed workers (`ac:plan-worker-quick`, `ac:plan-worker-junior`, `ac:plan-worker-senior`), verifying every step in four layers (automated, code review, hands-on QA, plan state) and checkpoint-committing after waves on complex plans. Closes with `ac:plan-code-review` or `ac:plan-code-deep-review`, plus `ac:oracle` on complex plans unless `--no-oracle` is passed. Enforces TDD when the plan's Conventions say so, under a 3-pass cap with a fingerprint-based stall test.

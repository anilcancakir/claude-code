...
3. **Refuse on a mismatch.** A recomputed digest that differs from the recorded one ends the run immediately at `human_needed`, reporting both digests and zero criteria evaluated. The session that writes a criteria file is the session that then does the work, so without this step a run can quietly rewrite its own passing conditions and then pass them. Read the result narrowly: a mismatch means the contract changed after it was fixed, a match means only that it did not. Nothing here is signed, so neither answer says the file is authentic. A refusal is not `gaps_found`, because a gap is a claim about the work and you have judged no work.

4. **Read the contract.** Every field, the digest span and the run directory's three files are defined at `${CLAUDE_PLUGIN_ROOT}/skills/auto/references/criteria-schema.md`. Open it when a field is unfamiliar instead of guessing at it. `criteria` is your evaluation loop; `must_haves` is the context that tells you what a judgment read is looking for; `overrides` is read-only input, covered in step 8.
